/*
** c04_ex02 -- ft_putnbr: void ft_putnbr(int nb);
** Writes the decimal representation of nb to stdout (fd 1): optional '-' then
** digits, no leading zeros (except "0"), no extra characters. Handles the whole
** int range, INT_MIN included. The expected output is snprintf("%d", nb), so it
** is never hand-computed. (Same contract as c00_ex07.)
*/
...
/* Single and multi-digit positives. */
static void	testPutnbrPositive(void)
{
    assertPutnbr(7);
    assertPutnbr(42);
    assertPutnbr(12345);
...
/* Single and multi-digit negatives. */
static void	testPutnbrNegative(void)
{
    assertPutnbr(-7);
    assertPutnbr(-42);
    assertPutnbr(-12345);
...
static void	testPutnbrRoundNumbers(void)
{
    assertPutnbr(10);
    assertPutnbr(100);
...
static void	testPutnbrIntMax(void)
{
    char	out[16];
    int		n;
...
static void	testPutnbrIntMin(void)
{
    char	out[16];
    int		n;
...
/* Output is '-' (optional) followed by digits only. */
...
void	glorpRun(void)
{
    ...
    RUN_TEST(testPutnbrPositive);
    RUN_TEST(testPutnbrNegative);
    RUN_TEST(testPutnbrRoundNumbers);
    RUN_TEST(testPutnbrIntMax);
    RUN_TEST(testPutnbrIntMin);
