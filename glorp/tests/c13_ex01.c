/*
** c13_ex01 -- btree_apply_prefix:
** void btree_apply_prefix(t_btree *root, void (*applyf)(void *));
** Applies applyf to every item using prefix traversal: the current node
** first, then the left subtree, then the right subtree.
*/
...
extern void	btree_apply_prefix(t_btree *root, void (*applyf)(void *));
...
static void	testPrefixSingleNode(void)
{
    char	item[] = "1";
    t_btree	leaf = {NULL, NULL, item};

    btree_apply_prefix(&leaf, recordItem);
    TEST_ASSERT_EQUAL_INT(1, gCallCount);
    TEST_ASSERT_EQUAL_PTR(item, gItems[0]);
}

/* Full 3-level tree shaped so prefix order (node, left, right) is "1".."7":
**             "1"
**           /     \
**        "2"       "5"
**       /   \     /   \
**     "3"   "4" "6"   "7"
** (infix would give 3 2 4 1 6 5 7, suffix 3 4 2 6 7 5 1, and level-order
** 1 2 5 3 4 6 7, so the expected order is unambiguous.) */
static void	testPrefixSevenNodeTree(void)
{
    ...
    btree_apply_prefix(&n1, recordItem);
    assertCallOrder(expected, 7);
}

/* A NULL root applies the function zero times. */
static void	testPrefixNullRootZeroCalls(void)
{
    btree_apply_prefix(NULL, recordItem);
    TEST_ASSERT_EQUAL_INT(0, gCallCount);
}

void	glorpRun(void)
{
    RUN_TEST(testPrefixSingleNode);
    RUN_TEST(testPrefixSevenNodeTree);
    RUN_TEST(testPrefixNullRootZeroCalls);
}
